...
"""
    ...
    Small script to verify moteus control law, and meaning of different variables

    Feel free to play around and print out the variables.
    - I noticited that torque and velocity have higher errors
    - It works, but its definetly not as exact as a simulator XD
...
async def main():
    ...
    while True:
        ...
        print("----------")
        verify('Position Error',  v[Register.POSITION] - v[Register.CONTROL_POSITION], v[Register.POSITION_ERROR])
        verify('Velocity Error', v[Register.VELOCITY] - v[Register.CONTROL_VELOCITY], v[Register.VELOCITY_ERROR], eps = 0.05)
        verify('Torque Error', v[Register.TORQUE] - v[Register.CONTROL_TORQUE], v[Register.TORQUE_ERROR], eps = 0.05)

        verify('P', v[Register.POSITION_ERROR] * curr_cmd.kp * _kp, v[Register.POSITION_KP])
        verify('D', v[Register.VELOCITY_ERROR] * curr_cmd.kd * _kd, v[Register.POSITION_KD])
        
        # It's expected that this will throw an error the first time, as the command needs to update in the system
        pid = v[Register.POSITION_KP] + v[Register.POSITION_KI] + v[Register.POSITION_KD] +  -1*v[Register.POSITION_FEEDFORWARD]

        if not math.isnan(curr_cmd.max_t):
            if pid < 0:
                pid = max(pid, -curr_cmd.max_t)
            elif pid > 0:
                pid = min(pid, curr_cmd.max_t)

        verify('PID', pid,-1 * v[Register.POSITION_COMMAND])
